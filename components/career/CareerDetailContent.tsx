          ...
          return (
            <section key={slot}>
              <div className="flex items-center gap-3 mb-4">
                <span className="w-7 h-7 rounded-lg flex items-center justify-center bg-white/[0.06] text-gray-400">
                ...
                <ul className="mt-5 space-y-2">
                  {visuals.bullets[locale].map((item, i) => (
                    <li key={i} className="flex items-baseline gap-3 text-sm text-gray-300 leading-relaxed">
                      <span className="shrink-0 w-1.5 h-1.5 mt-1.5 rounded-full bg-gray-600" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-3">
                  {visuals.metrics.map((m, i) => (
                    <div key={i} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                      ...
                      <div className="mt-1 text-[12px] text-gray-500 leading-snug">{m.label[locale]}</div>
                    </div>
                  ))}
                </div>
              )}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-white/[0.03]">
                        {visuals.table.columns.map((col, i) => (
                          <th key={i} className="text-left px-4 py-2.5 font-medium text-gray-400 border-b border-white/[0.06]">
                            {col[locale]}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {visuals.table.rows.map((row, i) => {
                        ...
                        return (
                          <tr key={i} className={`border-b border-white/[0.04] last:border-0 ${selected ? 'bg-white/[0.04]' : ''}`}>
                            {row.map((cell, j) => (
                              <td key={j} className={`px-4 py-3 ${selected ? 'text-white font-medium' : 'text-gray-400'}`}>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 20, right: 20, left: 10, bottom: 10 }} barCategoryGap="35%">
            ...
            <Bar dataKey="value" radius={[6, 6, 0, 0]} label={{ position: 'top', fill: 'rgba(255,255,255,0.7)', fontSize: 11 }}>
              {data.map((d, i) => (
                <Cell key={i} fill={d.isAfter ? '#22d3ee' : 'rgba(255,255,255,0.15)'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
