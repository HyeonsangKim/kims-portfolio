...
> = {
  ...
  custos: {
    ...
    context: {
      ko: '이 프로젝트는 Google Cloud Rapid Agent Hackathon 2026 GitLab Track 출품작으로, 트랙 제약상 GitLab 리포 위에서 도는 에이전트를 만드는 것이 출발점이었습니다. 우리가 잡은 질문은 단순했습니다. PR 리뷰 봇은 왜 하나같이 "리뷰까지"에서 멈추는가.\n\n기존 PR 리뷰 봇(Copilot Review, CodeRabbit, GitLab Duo)은 하나같이 수동적입니다. PR이 열려야 깨어나고, 머지되는 순간 그 코드를 잊어버립니다. 정작 사고는 머지된 뒤에 터지는데 말이죠. 좋은 시니어는 정반대입니다. 리포 건강을 끝까지 책임지고, 시간이 갈수록 팀 기준에 맞춰갑니다.\n\n그래서 단순히 리뷰만 잘하는 봇이 아니라, 머지된 뒤 실제 사고를 보고 스스로 기준을 고쳐가며 점점 시니어처럼 되어가는 커스토디언을 목표로 잡았습니다.',
      en: 'This was our entry for the Google Cloud Rapid Agent Hackathon 2026 GitLab Track, so the starting point was fixed by the track: build an agent that runs on top of a GitLab repo. The question we framed was simple — why do PR-review bots all stop at "reviewed"?\n\nExisting PR-review bots (Copilot Review, CodeRabbit, GitLab Duo) are all passive: they wake up when a PR opens and forget the code the moment it merges, even though incidents usually surface after the merge. A good senior is the opposite, owning repo health end to end and adjusting to the team’s standards over time.\n\nSo we aimed for more than a bot that reviews well: a custodian that watches real post-merge incidents, fixes its own standards from them, and grows into a senior over time.',
      ja: '本プロジェクトはGoogle Cloud Rapid Agent Hackathon 2026 GitLab Trackの出品作で、トラックの制約上GitLabリポジトリ上で動くエージェントを作ることが出発点でした。私たちが立てた問いはシンプルです。PRレビューボットはなぜどれも「レビューまで」で止まるのか。\n\n既存のPRレビューボット（Copilot Review、CodeRabbit、GitLab Duo）はどれも受動的です。PRが開くと目覚め、マージされた瞬間にそのコードを忘れます。実際の事故はマージ後に起きるのに。良いシニアは正反対で、リポの健全性を最後まで担い、時間とともにチームの基準に合わせていきます。\n\nそこで、単にレビューが上手いボットではなく、マージ後の実際の事故を見て自ら基準を直し、次第にシニアになっていくカストディアンを目標にしました。',
    },
    problem: {
